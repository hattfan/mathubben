...
function drawBar(data, KommunKod) {
  var bar = d3.select("#bar");
  var padding = {
    top: 30,
    right: 30,
  ...
  //! Måste hitta samma för vanliga
  var kommunData = data.filter(d => d.Kommunkod === KommunKod);

  var xScale = d3.scaleLinear()
                 .domain(d3.extent(data, d => d.Kommunkod))
                 .range([padding.left, width - padding.right]);

  var yScale = d3.scaleLinear()
                 .domain([0, d3.max(kommunData, d => d.Value)])
  ...
  update
    ...
    .append("rect")
      ...
      .attr("height", 0)
    .merge(update)
      .attr("x", d => (xScale(d.Kommunkod) + xScale(d.Kommunkod - 1)) / 2)
